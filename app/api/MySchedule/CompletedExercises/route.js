  ...
  try {
    ...
    if (!userId || !date || !day) {
        return new NextResponse(
          JSON.stringify({ message: "Missing required query parameters: userId, date, or day." }),
          { status: 400 }
        );
    }
    ...
    if (!userSchedule) {
      return new NextResponse(
        JSON.stringify({ message: "No workout log found for the specified date and day." }),
        { status: 404 }
      );
    }

    return new NextResponse(JSON.stringify(userSchedule.exercises_done[0]), { status: 200 });
  } catch (error) {
    return new NextResponse(
